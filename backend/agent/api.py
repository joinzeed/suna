...
class AgentCreateRequest(BaseModel):
    ...
    system_prompt: Optional[str] = "You are a helpful assistant. Provide clear, accurate, and helpful responses to user queries."
...
async def create_agent(
    ...
    logger.info(f"Received agent data: name={agent_data.name}, system_prompt={agent_data.system_prompt}")
    if not await is_enabled("custom_agents"):
        raise HTTPException(
            status_code=403, 
            detail="Custom agents currently disabled. This feature is not available at the moment."
